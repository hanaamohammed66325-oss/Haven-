-- ---------------------------------------------------------------------------
-- admin_gpa_checks() also returns the cumulative GPA checks (her request
-- 2026-09-26, "ضيف خيار للتراكمي هو الاهم"): term_cum_checked (the cumulative
-- GPA after this term against the portal's) and term_cum_reason (why the
-- student thinks it differed). Past terms carry theirs inside
-- past_term_checked. Both start with term_ so a withdrawn consent hides their
-- details like the other end-of-term events. Same guard and grants as before.
-- ---------------------------------------------------------------------------
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
   ...
   where e.event in (
     ...
     'term_cum_checked', 'term_cum_reason'
   );
  return res;
end;
$$;
...
-- Rollback: re-run admin_gpa_checks() from 20260927_gpa_checks_repeat_rules.sql.
